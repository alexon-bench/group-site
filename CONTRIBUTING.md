...
4. Keep both protected template sections unchanged: `MTECH TOP BAR STYLES
   START` through `MTECH TOP BAR STYLES END`, and `MTECH TOP BAR START` through
   `MTECH TOP BAR END`.
5. If the request says to add, publish, or integrate the tool, copy one card
   ...
   `index.html`. A tool is not integrated until this card exists.
...
- The landing page includes exactly one card for every integrated tool.
- Only the expected files changed.

GitHub automatically runs the same tool-and-navigation check on proposed and
pushed changes. If it turns red, open its message; it names the missing card
